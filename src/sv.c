...
char *action;
char *acts;
char *varservice ="/data/adb/runsvdir/service/";
char **service;
char **servicex;
...
int main(int argc, char **argv) {
  ...
  *argv +=i;
  ...
  if ((x =env_get("SVWAIT"))) scan_ulong(x, &wtime);
  while ((i =getopt(argc, (char* const*)argv, "w:vV")) != opteof) {
    switch(i) {
    case 'w': scan_ulong(optarg, &wtime);
  ...
  if (*cbk)
    for (;;) {
      ...
      for (i =0; i < services; ++i, ++service) {
        ...
        if (*service && taia_approx(&tdiff) > wtime) {
